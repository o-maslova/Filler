...
void	you_win(t_win *win)
{
	...
	VIEW_COLUMN * CELL_SIZE + PAD + 20, 0xFFD700, "Your score: ");
	mlx_string_put(win->mlx_ptr, win->win_ptr, POSITION + 100,
	VIEW_COLUMN * CELL_SIZE + PAD + 20, 0xFFD700, my_score);
	mlx_string_put(win->mlx_ptr, win->win_ptr, POSITION - 40,
	VIEW_COLUMN * CELL_SIZE + PAD + 40, 0xFFD700, "Enemy score: ");
	mlx_string_put(win->mlx_ptr, win->win_ptr, POSITION + 110,
	VIEW_COLUMN * CELL_SIZE + PAD + 40, 0xFFD700, enemy_score);
	mlx_string_put(win->mlx_ptr, win->win_ptr, POSITION,
	VIEW_COLUMN * CELL_SIZE + PAD + 60, 0x00FF00, "YOU WIN!");
	system("say you, win");
}
...
void	you_lose(t_win *win)
{
	...
	VIEW_COLUMN * CELL_SIZE + PAD + 40, 0xFFD700, enemy_score);
	mlx_string_put(win->mlx_ptr, win->win_ptr, POSITION,
	VIEW_COLUMN * CELL_SIZE + PAD + 60, 0xFF0000, "YOU LOSE!");
	system("say you, lose");
}
